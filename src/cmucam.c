...
int
camera_set_reg (int reg, int val)
{
  unsigned int data[3];
  int to;
  data[0] = camera_type;
...
void
image_fifo_to_mem (unsigned char *img, int size_x, int size_y)
{
  int x, y;
...
void
image_send_uart (unsigned char *img, int size_x, int size_y)
{
  int y, x;
...
/*void FRAME_READ_INC()
{
//while(REG(GPIO_IOPIN)&0x8000);
	REG(GPIO_IOSET)=0x800; 
	REG(GPIO_IOCLR)=0x800; 
}*/
...
fifo_read_pixel ()
{
  ...
  // Help combat bad contacts in the bus? and because if you take them away the compiler breaks
  // If you remove this, the frame rate goes up on the ov6620 while the ov7620 could have problems
 /*  if(red_pixel<16) red_pixel=16;
     if(red_pixel>240) red_pixel=240;
     if(green_pixel<16) green_pixel=16;
     if(green_pixel>240) green_pixel=240;
     if(blue_pixel<16) blue_pixel=16;
...
void
set_cam_ddr (volatile unsigned long val)
{
  //DDR(I2C_PORT,val);
  REG (GPIO_IODIR) = val;
  delay_i2c ();
...
unsigned int
i2c_send (unsigned int num, unsigned int *buffer)
{
