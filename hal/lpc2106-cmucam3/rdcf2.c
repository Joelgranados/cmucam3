...
static void flush_buffer(struct rdcf *f)
{
	if (f->buffer_status == DIRTY) {
		...
		if ((f->sector_in_buffer >= f->first_FAT_sector) &&
				...
				// mirror.
			write_sector(f, f->sector_in_buffer+f->sectors_per_FAT, f->buffer.buf);
		}
...
static void check_file_character(struct rdcf *f, unsigned c)
{
	...
	if (table[c>>3] & 1<<(c&7)) error_exit(f, ~EINVAL);
}
	...
	uint8_t *name_extension, const uint8_t *spec)
{
	...
	if (i==0) error_exit(f, ~EINVAL);
	while (i<NAME_SIZE) name_extension[i++] = ' ';
	if (c=='.') while ((c=(*spec++))!=0 && c!=RDCF_SLASH_CHAR) {
		check_file_character(f,c);
...
static void clear_cluster(struct rdcf *f, unsigned cluster)
{
	...
	do write_sector(f, sector++, f->buffer.buf); while (--count != 0);
}
...
{
	if (f->directory_index == NO_DIRECTORY_INDEX) {
		if (f->directory_cluster == 0) error_exit(f, ~ENOSPC);
		...
		while (cluster != EMPTY_CLUSTER && cluster < f->last_cluster_mark) {
			...
			do {
				...
				do {
					...
					if (c != DELETED_FILE && c != '.') error_exit(f, ~EACCES);
